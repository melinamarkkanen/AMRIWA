...
## Data exploration using library HighstatLabv13
...
#boxplot(E1 ~ country, data = df,  ylab = "Residuals")
#abline(h = 0)
# Looks good.
...
# Add the p values obtained above
...
## Plot models by hospitals / hospital sections (ARGs)
...
# Fit model
M4 <- glm(ARG_SUM ~ hospital_section,
           data = df, family="Gamma"(link="log"))
summary(M4)

glht.M4 <- glht(M4, mcp(hospital_section = "Tukey"))
summary(glht(glht.M4))
```
...
# Multiply with SSU counts from Metaxa2
...
# Heatmap for clinically relevant taxa
...
## Most abundant ARGs in other than HWW samples
...
# Correlation between MGE/intI1 & all ARGs
...
cor <- ggplot(mge_res, aes(x=ARGs, y=MGEs)) +
  ...
  theme(axis.title = element_text(size = 30, family = "Times"),
        ...
        plot.subtitle = element_text(size = 28, family = "Times")) +
 xlab("ARG") + ylab("MGEs") +
  labs(title= "Correlation of relative sums of ARGs and MGEs",)
...
## Save correlation data for intI & qacEdelta and all ARGs
...
## Figures for correlations for differentially abundant ARGs across countries (from DESeq2) & intI1/qacEdelta
...
rownames(data) <- gsub(pattern = "_[A-Z].*", replacement = "", rownames(data))
rownames(data) <- gsub(pattern = "-", replacement = "_", rownames(data))
...
cor_annotation <- ggcorrplot(r, method = "square", outline.col = "grey", legend.title = "intI1")
