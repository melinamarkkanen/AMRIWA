...
rfc_plot <- rfc +
  ...
  scale_x_discrete(breaks=levels(factor(rownames(sample_data(resfinder_PHY_Cluster)))), labels=alias, expression(bar("x"))) +
  theme_minimal() +
...
rfc_plot1 <- rfc +
  ...
  theme(strip.text.x= element_text(size = 30, family = "Times", hjust = 0, vjust = 0.5, angle = 0),
        strip.background = element_rect(colour = "white")) + guides(fill=guide_legend(ncol=1))
rfc_plot1
...
rfc_plot2 <- rfc +
  ...
  theme(strip.text.x= element_text(size = 26, family = "Times", hjust = 0, vjust = 0.5, angle = 0),
        strip.background = element_rect(colour = "white")) + guides(fill=guide_legend(ncol=1))
rfc_plot2
...
a <- plot_grid(rfc_plot1, 
          rfc_plot2 + theme(axis.title.y = element_blank()), 
          rfc_plot3 + theme(axis.title.y = element_blank()), nrow = 1, coord_fixed())
a
#ggsave(filename = "carbapenemases_all.png",
#       width = 16, height = 13, dpi = 300, units = "in", device='png', scale = 1) 
...
rfc_plot4 <- rfc +
  ...
  theme(axis.text.x = element_text(size = 20, family = "Times", angle = 0, face = "bold"),
        ...
        panel.grid.major = element_blank(),
        plot.title = element_text(size = 30, family = "Times", face = "bold")) +
  facet_grid(~plot_name, scales = "free", space = "free", labeller = label_wrap_gen(width = 50, multi_line = TRUE)) +
  theme(strip.text.x= element_text(size = 22, family = "Times", hjust = 0, vjust = 0.5, angle = 0),
        strip.background = element_rect(colour = "white")) + guides(fill=guide_legend(ncol=1))
rfc_plot4
...
# Specific ARGs
resfinder_PHY_feces_clust <- tax_glom(resfinder_PHY_feces, taxrank = "Cluster_name")
resfinder_PHY_feces_nim <- subset_taxa(resfinder_PHY_feces_clust, Cluster_name == "nimE_1_clust")
resfinder_PHY_feces_nim <- subset_taxa(resfinder_PHY_feces_nim, taxa_sums(resfinder_PHY_feces_nim) != 0)

resfinder_PHY_feces_nim <- subset_taxa(resfinder_PHY_feces_clust, Cluster_name == "nimA_1")
resfinder_PHY_feces_nim <- subset_taxa(resfinder_PHY_feces_nim, taxa_sums(resfinder_PHY_feces_nim) != 0)
