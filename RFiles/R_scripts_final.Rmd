...
resfinder_M0 <- ggplot(dfA, aes(x = country, y = Mean)) + scale_color_manual(values=cols) + 
  ...
  geom_errorbar(aes(ymin = Mean - SE, ymax = Mean + SE), width = 0.5, lwd = 0.75) + geom_point(size = 0.9) + 
  theme_linedraw() + 
  theme(axis.text.x = element_text(angle = 0, size = 50, family = "Times", face = "bold"), 
        axis.title.x = element_blank(),
        axis.text.y = element_text(size = 36, family = "Times"),
        axis.title.y = element_text(size = 36, family = "Times"),
        legend.title = element_blank(),
        legend.text = element_text(size = 28, family = "Times"),
        plot.title = element_text(size = 46, family = "Times", face = "bold")) +
  labs(y = "ARGs normalized to 16S rRNA", x = "") + 
  guides(color = FALSE, alpha = FALSE) + 
  labs(title = "Sum relative abundance of ARGs (ResFinder)")
resfinder_M0 + stat_pvalue_manual(pvalues, label = "p", y.position = 2.3, step.increase = 0.05, tip.length = 0.01, size = 12)
...
MGE_M0 + stat_pvalue_manual(pvalues, label = "p", y.position = 7, step.increase = 0.05, tip.length = 0.01, size = 12)
...
MGE_M0 + stat_pvalue_manual(pvalues, label = "p", y.position = 1.05, step.increase = 0.05, tip.length = 0.01, size = 12)
...
metaphlan.p_ord <- p_ord + 
  ...
  stat_ellipse(level = 0.90, linetype = 1) +
#    geom_text_repel(mapping = aes(label = hospital), size = 7, family = "Times", hjust = 1.2) +
    theme_minimal() + labs(title= "Taxonomical composition (Metaphlan3)", subtitle = "Hospital WWs in Benin, Burkina Faso and Finland") + 
    theme(plot.title = element_text(size = 36, family = "Times", face = "bold"),
          plot.subtitle = element_text(size = 20, family = "Times"),
...
arg <- p_ord + 
  ...
  stat_ellipse(level = 0.90, linetype = 1) +
    geom_text_repel(mapping = aes(label = hospital), size = 7, family = "Times", hjust = 1.2) +
    theme_minimal() + labs(title = "Resistome (ResFinder)") +
...
mp <- p_ord + 
  ...
  geom_point(size = 3.5) + 
  stat_ellipse(level = 0.90, linetype = 1) +
    geom_text_repel(mapping = aes(label = hospital), size = 5.5, family = "Times", hjust = 1.2) +
    theme_minimal() + labs(title= "Taxonomical composition (Metaphlan3)") + 
...
mge <- p_ord + 
  ...
  stat_ellipse(level = 0.90, linetype = 1) +
  geom_text_repel(mapping = aes(label = hospital), size = 5.5, family = "Times", hjust = 1.2) +
   theme_minimal() + labs(title= "Mobilome (MGE database)") +
    theme(plot.title = element_text(size = 20, family = "Times", face = "bold"),
          legend.position = "none",
          axis.title = element_text(size = 18, family = "Times"),
          axis.text = element_text(size = 18, family = "Times")) +
...
#arg + plot_layout(guides = 'collect', widths = c(10), heights = c(10)) | (mp / mge) + 
#                    plot_layout(guides = 'collect', widths = c(1, 1), heights = c(1, 1))

#arg + plot_layout(guides = 'keep', widths = c(10), heights = c(10)) | (mp / mge) + 
#                    plot_layout(guides = 'keep', widths = c(1, 1), heights = c(1, 1))

#arg + plot_layout(guides = 'keep', widths = c(10), heights = c(10)) | (mp / mge) + 
#                    plot_layout(guides = 'keep', widths = c(1), heights = c(1, 1), ncol = 1, nrow = 1)

mp_mge <- plot_grid(mp, mge, nrow = 2, labels = c("B", "C"))
arg_label <- plot_grid(arg, labels = c("A"), nrow = 1)
ordinations <- plot_grid(arg_label, mp_mge, ncol = 2, scale = c(1.15, .75, 75))
ordinations

plot_grid(ordinations, ncol = 1) + theme(plot.margin = unit(c(0,0,0,2), "cm"))

# OR

((arg + plot_layout(guides = 'keep')) | mp / mge) + plot_layout(guides = 'keep') + 
...
heat <- pheatmap(sqrt(new_df), cluster_rows = F, cluster_cols = T, 
                        ...
                        main = "Relative abundance of clinically significant species,\n (Metaphlan3, square root transormed)", 
...
rfc_plot <- rfc +
  ...
  scale_x_discrete(breaks=levels(factor(rownames(sample_data(resfinder_PHY_Cluster)))), labels=hospital, expression(bar("x"))) +
  theme_minimal() +
  theme(axis.text.x = element_text(size = 16, family = "Times", angle = 0, hjust = 1, vjust = 1),
        axis.text.y = element_text(size = 28, family = "Times", face = "bold", angle = 0),
        axis.title.y = element_text(size = 30, family = "Times"),
        axis.title.x = element_blank(),
  #      legend.text = element_text(size = 30, family = "Times", face = "italic"),
  #      legend.title = element_blank(),
...
rfc_plot1 <- rfc +
  ...
  scale_x_discrete(breaks=levels(factor(rownames(sample_data(resfinder_PHY_feces)))), labels=names, expression(bar("x"))) +
  theme_minimal() +
  theme(axis.text.x = element_text(size = 22, family = "Times", angle = 0, face = "bold"),
        axis.text.y = element_text(size = 22, family = "Times", face = "bold", angle = 0),
        axis.title.y = element_blank(),
        axis.title.x = element_blank(),
        legend.position = "none",
        panel.background = element_rect(fill = "#FFFDF9"),
...
#(rfc_plot1 / rfc_plot2 / rfc_plot3 + plot_layout(guides = 'keep')) + plot_annotation(tag_levels = list(c("B", "C", "D"))) &
#  theme(plot.tag = element_text(size = 24, family = "Times"))

#((rfc_plot1 / rfc_plot2 / rfc_plot3 + plot_layout(guides = 'keep')) | rfc_plot4) +
...
DD##
"
other <- rfc_plot1 + rfc_plot2 + rfc_plot3 + rfc_plot4 + 
  plot_layout(design = layout) + plot_annotation(tag_levels = list(c("B", "C", "D", "E"))) &
  theme(plot.tag = element_text(size = 24, family = "Times"))

other_leg <- other + inset_element(leg, left = 0.5, bottom = -3.25, right = 0.5, top = 0)
other_leg

ggsave(filename = "carbapenemases_grid.png",
       width = 16, height = 13, dpi = 300, units = "in", device='png', scale = 1) 

rfc_plot + plot_layout(guides = 'keep') + plot_annotation(tag_levels = "A")  &
  theme(plot.tag = element_text(size = 24, family = "Times"))
