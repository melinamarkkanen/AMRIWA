...
gps_Ben <- subset(gps, country=="Benin")

# Add important cities
gps_Ben_labels <- data.frame(country = c("Benin", "Benin", "Benin"),
                               lat = c("6.4990718", "6.3676953", "9.3400159"),
                               long = c("2.6253361", "2.4252507", "2.6278258"),
                             hospital = c("nd", "nd", "nd"))
rownames(gps_Ben_labels) <- c("Porto Novo", "Cotonou", "Parakou")

gps_data <- rbind(gps_Ben, gps_Ben_labels)
gps_data$Label <- c("nd", "nd", "nd", "nd", "nd", "Porto Novo", "Cotonou", "Parakou")
gps_data$lat <- as.numeric(gps_data$lat)
gps_data$long <- as.numeric(gps_data$long)

Ben_map <- ggplot(data = world) +
    geom_sf() +
    ...
    geom_point(data = subset(gps_data, Label != "nd"), 
               aes(x = long, y = lat), size = 3, shape = 9, color = "black") +
    geom_label_repel(data=subset(gps_data, Label !="nd"),  
                     aes(x = long, y = lat, label = Label, family = "Times"), color = "black", size = 5) +
  coord_sf(ylim = c(5.5, 12.5), xlim = c(0.5, 4), expand = T) + 
...
Ben_map <- Ben_map + theme(plot.margin = ggplot2::margin(0, 0, 0, 0, "cm")) +
  scale_x_continuous(breaks = seq(0, 4, 1)) +
  inset_element(title_Ben, left = 0.15, bottom = 0.35, right = 1, top = 1)

#title_Ben <- ggdraw(get_title(Ben_map))
...
gps <- metadata[!duplicated(metadata[,c('lat','long')]),]
gps <- gps[ , c("country", "lat", "long", "hospital")] 
gps_BF <- subset(gps, country=="Burkina Faso")

# Add important cities
gps_BF_labels <- data.frame(country = c("Burkina Faso", "Burkina Faso"),
                               lat = c("12.3681873", "11.1757783"),
                               long = c("-1.5270944", "-4.2957591"),
                            hospital = c("nd", "nd"))
rownames(gps_BF_labels) <- c("Ouagadougou", "Bobo Dioulasso")

gps_data <- rbind(gps_BF, gps_BF_labels)
gps_data$Label <- c("nd", "nd", "nd", "nd", "Ouagadougou", "Bobo Dioulasso")
gps_data$lat <- as.numeric(gps_data$lat)
gps_data$long <- as.numeric(gps_data$long)

BF_map <- ggplot(data = world) +
    geom_sf() +
    borders("world", colour="black", fill="wheat1")  +
    theme(panel.background = element_rect(fill = "azure1", colour = "azure1")) +
    geom_point(data = subset(gps_data, Label == "nd"), 
    ...
    geom_label_repel(data=subset(gps_data, Label !="nd"),  
                     aes(x = long, y = lat, label = Label, family = "Times"), color = "black", size = 5) +
  coord_sf(ylim = c(9, 15), xlim = c(-6, 2.5), expand = T) + 
  theme(axis.text = element_text(family = "Times", size = 16),
#        plot.title = element_text(family = "Times", face = "bold", size = 28, hjust = 0.5, vjust = 0.5, colour = "#4C4B49"),
        axis.title = element_blank()) +
...
BF_map <- BF_map + theme(plot.margin = ggplot2::margin(0, 0, 0, 0, "cm")) + 
  inset_element(title_BF, left = 0.15, bottom = 0.45, right = 1, top = 1)

#title_BF <- ggdraw(get_title(BF_map))

layout <- "
AB
"
p_map <- Ben_map + BF_map + 
  plot_layout(design = layout, widths = c(0.5, 1.4))
...
# Add important cities
gps_labels <- data.frame(country = c("country_name", "Benin", "Benin", "Benin", "country_name", "Burkina Faso", "Burkina Faso", "ocean"),
                               lat = c("10.544904033009432", "6.4990718", "6.3676953", "9.3400159", "13.740788326149952",  "12.3681873", "11.1757783", "4.944956754100344"),
                               long = c("2.3165032566686428", "2.6253361", "2.4252507", "2.6278258", "-1.0794179365270806", "-1.5270944", "-4.2957591", "2.376996878456601"),
                             hospital = c("nd", "nd", "nd", "nd", "nd", "nd", "nd", "nd"))
rownames(gps_labels) <- c("Benin", "Porto Novo", "Cotonou", "Parakou", "Burkina Faso", "Ouagadougou", "Bobo Dioulasso", "Gulf of Guinea")

gps_data <- rbind(gps, gps_labels)
gps_data$Label <- c("nd", "nd", "nd", "nd", "nd", "nd", "nd", "nd", "nd", 
                    "Benin", "Porto Novo", "Cotonou", "Parakou", "Burkina Faso", "Ouagadougou", "Bobo Dioulasso", "Gulf of Guinea")
gps_data$lat <- as.numeric(gps_data$lat)
gps_data$long <- as.numeric(gps_data$long)
...
p_map1 <- ggplot(data = world) +
    ...
    geom_text_repel(data = subset(gps_data, Label == "nd"), 
                    mapping = aes(x = long, y = lat, label = hospital, family = "Times"), size = 5, 
                    #hjust = -1.3, vjust = -0.1,
                    point.padding = 1e-06) +
...
p_map2 <- p_map1 + 
  ...
  geom_label_repel(data=subset(gps_data, Label == "Porto Novo" | Label == "Cotonou" | Label == "Parakou" | 
                             Label == "Ouagadougou" | Label == "Bobo Dioulasso"),  
                      aes(x = long, y = lat, label = Label), color = "black", size = 5, family = "Times", 
                   box.padding = 0.95)
p_map2
...
p_map3 <- p_map2 + 
  ...
  geom_text_repel(data = subset(gps_data, Label == "Benin" | Label == "Burkina Faso" | Label == "Gulf of Guinea"),
                  ...
                  color = "#4C4B49", size = 10, family = "Times")
p_map3
...
p1 <- xyplot(long ~ lat, aspect = "fill", col = "#B2182B", pch = 19, data = df_Benin, cex = 3.5, par.settings=mytheme,
        ylab = "Latitude", xlab = "Longitude", main = "Benin",
    panel = function(x, y, ...) {
    panel.grid(h = -1, v = -1, col.line = grey(0.9))
    panel.xyplot(x, y, ...)
...
p2 <- xyplot(lat ~ long, aspect = "fill", col = "#44AA99", pch = 19, data = df_BF, cex = 3.5, par.settings=mytheme,
               ylab = "Latitude", xlab = "Longitude", main = "Burkina Faso",
    panel = function(x, y, ...) {
...
p3 <- xyplot(lat ~ long, aspect = "fill", col = "#2166AC", pch = 19, data = df_Finland, cex = 3.5, par.settings=mytheme,
               ylab = "Latitude", xlab = "Longitude", main = "Finland",
    panel = function(x, y, ...) {
    panel.grid(h = -1, v = -1, col.line = grey(0.9))
    panel.xyplot(x, y, ...)
