...
metaxa_rel_PHY <- transform_sample_counts(metaxa_PHY_ast, function(x) x/sum(x))
...
metaxa.p1 <- p1 + scale_color_manual(values=c("#FF333F", "#531592", "#35E0F5")) + geom_point(colour = "black", 
...
metaxa_temp <- subset_samples(metaxa_PHY_ast, (country == "Benin" | country == "Finland"))
...
metaxa_temp <- subset_samples(metaxa_PHY_ast, (country == "Burkina Faso" | country == "Finland"))
metaxa_dist <- vegdist(t(otu_table(metaxa_temp)), dist = "horn")
adonis(metaxa_dist ~ country, data = data.frame(sample_data(metaxa_temp), permutations = 9999))

metaxa_temp <- subset_samples(metaxa_PHY_ast, (country == "Benin" | country == "Burkina Faso"))
metaxa_dist <- vegdist(t(otu_table(metaxa_temp)), dist = "horn")
adonis(metaxa_dist ~ country, data = data.frame(sample_data(metaxa_temp), permutations = 9999))
...
```{r}
# Ordinate and plot data
metaphlan_PHY_ord <- ordinate(metaphlan_PHY_ast, method = "PCoA", distance = "horn")
p2 <- plot_ordination(metaphlan_PHY_ast, metaphlan_PHY_ord, color = "country", label = "name")
metaphlan.p2 <- p2 + scale_color_manual(values=c("#FF333F", "#531592", "#35E0F5")) + 
...
resfinder.p3 <- p3 + scale_color_manual(values=c("#FF333F", "#531592", "#35E0F5")) +
    geom_point(colour = "black", pch = 21, size = 2, alpha = 0.5) + stat_ellipse(level = 0.95, linetype = 1) +
...
resfinder_PHY_ord <- ordinate(resfinder_PHY_ast, method = "PCoA", distance = "horn")
p3.1 <- plot_ordination(resfinder_PHY_ast, resfinder_PHY_ord, color = "location", label = "name")
...
# Test significance between all pairs
resfinder_temp <- subset_samples(resfinder_PHY_ast, (location == "Savalou area" | location == "Central clinic of Calavi"))
resfinder_dist <- vegdist(t(otu_table(resfinder_temp)), dist = "horn")
...
# Tecnichal replicates seem highly similar to each other. Fecal samples have different diversity in the class taxonomy: more Closridia and Betaproteobacteria than Gammaproteobacteria and no Epsilonproteobacteria when comparing to ww samples. Of the suscpicious samples especially in BH02 the Gammaproteobacteria are dominating. This would make sense if the sample was from CLED plate which is selective for Enterobacteriaceae species. 
```
## Top 15 genera merged by location, Metaphlan3
```{r}
metaphlan_PHY_genus <- tax_glom(metaphlan_PHY_ast, taxrank = "Genus")
...
otu_table(merged) <- otu_table(merged)[, ]/as.matrix(table(sample_data(metaphlan_PHY_ast)$location))[, 
    1]
...
metaphlan.p16 <- plot_bar(merged, fill = "Genus", title = "15 most abundant genera, Metaphlan3") +
...
#rownames(metadata)
#SSU<-metadata[!(metadata$name=="BH02_S77" | metadata$name=="BH27_S92" | metadata$name=="BH30_S94"),]

mean16S_counts <- mean(metadata$SSU_counts)
lo_data <- data.frame(SUM = mean16S_counts * (sample_sums(otu_table(resfinder_PHY_ast))), location= sample_data(resfinder_PHY_ast)$location)
...
co_data <- data.frame(SUM = round(mean16S_counts * (sample_sums(otu_table(resfinder_PHY_ww)))), 
    country = sample_data(resfinder_PHY_ww)$country)
...
```
## Top 12 ARG genes, sum abundance (Not correct)
```{r}
...
## Plot
pheatmap(heat_num, cluster_rows = F, cluster_cols = T, cellwidth = 5.0, cellheight = 4.5, border_color = "black", fontsize=5, color= inferno(length(mat_breaks) -1), breaks= mat_breaks, annotation_colors = ann_colors, annotation_col = annot_col, main = "x")
```
## crAssphage
```{r}
...
# Plot
p23<-plot_bar(crass_PHY, fill="sample_type")
...
# Remove OTU separators
crassphage <- crass.p23 + geom_bar(aes(fill=sample_type), stat="identity", position="stack")
crassphage
...
p24<-plot_bar(resfinder_PHY_Class_abund, x="name", fill="sample_type")
