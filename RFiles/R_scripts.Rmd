...
```{r}
# Ordinate and plot data
metaphlan_PHY_ord <- ordinate(metaxa_PHY, method = "PCoA", distance = "horn")
p2 <- plot_ordination(metaxa_PHY, metaphlan_PHY_ord, color = "country", label = "sample_type")
...
# Test significance using pair-wise adonis
metaphlan_temp <- subset_samples(metaxa_PHY, (country == "Benin" | country == "Finland"))
...
metaphlan_temp <- subset_samples(metaxa_PHY, (country == "Burkina Faso" | country == "Finland"))
metaphlan_dist <- vegdist(t(otu_table(metaphlan_temp)), dist = "horn")
adonis(metaphlan_dist ~ country, data = data.frame(sample_data(metaphlan_temp), permutations = 9999))

metaphlan_temp <- subset_samples(metaxa_PHY, (country == "Burkina Faso" | country == "Benin"))
metaphlan_dist <- vegdist(t(otu_table(metaphlan_temp)), dist = "horn")
adonis(metaphlan_dist ~ country, data = data.frame(sample_data(metaphlan_temp), permutations = 9999))
...
metaphlan_temp <- subset_samples(metaxa_PHY, (sample_type == "waste water" | sample_type == "soil"))
metaphlan_dist <- vegdist(t(otu_table(metaphlan_temp)), dist = "horn")
adonis(metaphlan_dist ~ sample_type, data = data.frame(sample_data(metaphlan_temp), permutations = 9999))

metaphlan_temp <- subset_samples(metaxa_PHY, (sample_type == "waste water" | sample_type == "river water"))
metaphlan_dist <- vegdist(t(otu_table(metaphlan_temp)), dist = "horn")
adonis(metaphlan_dist ~ sample_type, data = data.frame(sample_data(metaphlan_temp), permutations = 9999))
...
# With resfinder_PHY_ast negative model fits the data.
```
...
# Normalize by number of samples in each location
...
# Let's try to create custom gene set
resfinder_PHY_Gene <- tax_glom(resfinder_PHY_ast, taxrank = "Gene")
resfinder_PHY_custom = subset_taxa(resfinder_PHY_Gene, Gene =="aadA2_1_NC_010870" | Gene == "blaCMY-2_1_X91840")
...
heat_OTU = as(otu_table(resfinder_PHY_Gene), "matrix")
# transpose if necessary
if(taxa_are_rows(resfinder_PHY_Gene)){heat_OTU <- t(heat_OTU)}
...
# Extract abundance matrix from the phyloseq object
heat_tax = as(tax_table(resfinder_PHY_Gene), "matrix")
# Transpose if necessary
if(taxa_are_rows(resfinder_PHY_Gene)){heat_tax <- t(heat_tax)}
