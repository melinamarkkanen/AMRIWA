...
lo_data<-data.frame(SUM=round(mean(sample_data(resfinder_PHY_ww)$SSU_counts)*(sample_sums(resfinder_PHY_ww))), location=as.factor(sample_data(resfinder_PHY_ww)$location))
...
co_data<-data.frame(SUM=round(mean(sample_data(resfinder_PHY_ww)$SSU_counts)*(sample_sums(resfinder_PHY_ww))), country=as.factor(sample_data(resfinder_PHY_ww)$country))
...
```{r}
resfinder_PHY_Class <- tax_glom(resfinder_PHY_ww, taxrank = "Class")
...
```{r}
resfinder_PHY_Gene <- tax_glom(resfinder_PHY_ww, taxrank = "Gene")
...
# Betalactams
```{r}
resfinder_PHY_betalactams = subset_taxa(resfinder_PHY_ww, Class=="Betalactam")
any(taxa_sums(resfinder_PHY_betalactams) == 0)
metaxa_PHY_ww <- prune_taxa(taxa_sums(resfinder_PHY_betalactams) > 0, resfinder_PHY_betalactams)
any(taxa_sums(resfinder_PHY_betalactams) == 0)
...
# Metronidazole
```{r}
resfinder_PHY_metronidazole = subset_taxa(resfinder_PHY_ww, Class=="Metronidazole")
