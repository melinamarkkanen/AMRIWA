...
```
# Testing wheter the taxonomic composition is grouped significantly by location in each country, Metaxa2
```{r}
...
# In Benin the ARG profile is explained by  25.0 % by the location. Whereas in Burkina Faso the same percentage is  24.2 %. In Finland the grouping by location is not significant.
```
...
# The explanatory variable "country" explains only very little of the difference in the grouping of taxonomy between. Instead, sample types other than waste water seem to group together. Let's test if this is significant.
...
# In Benin the ARG profile is explained by  25.5 % by the location. Whereas in Burkina Faso the same percentage is  22.2 %. In Finland the grouping by location is not significant.
```
...
metaphlan_tab <-microbiome::alpha(metaphlan_PHY, index = "diversity_shannon")
...
# When using gini simpson the differences are significant. It is less likely that two individuals randomly selected from a sample will belong to different species among Finnish samples compared to BF and Beninise. -> Check outlier.
```
...
heat.metaphlan
```
# DRAFT Boxplot of relative abundance of top 10 genera
```{r}
...
metaphlan_box <- plot_taxa_boxplot(metaphlan_PHY_genus,
                        ...
                        title = "Relative abudance plot", color = "Dark2")
print(metaphlan_box)
# Error: Error in as(x, "matrix")[i, j, drop = FALSE] : subscript out of bounds -> Fix
```
...
summary(glht(glht.mod))
```
# Testing wheter the resistomes are grouped significantly by location in each country
```{r}
