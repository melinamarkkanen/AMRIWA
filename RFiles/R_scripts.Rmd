...
```
# Genera merged by location, Metaphlan3
```{r}
...
# Merge by location
mrg_loc <- merge_samples(metaphlan_PHY_genus_abun, "location")

# Normalize by sample number / location
otu_table(mrg_loc) <- otu_table(mrg_loc)[, ]/as.matrix(table(sample_data(metaphlan_PHY)$location))[, 
    1]
...
# Plot
metaphlan.p15 <- plot_bar(mrg_loc, fill = "Genus", title = "12 most abundant genera") + 
