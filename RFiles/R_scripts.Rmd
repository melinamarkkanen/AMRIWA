...
ARG_genemat_norm <- t(t(ARG_length_norm)/metadata$SSU_counts) * 1541
...
resfinder_PHY_hosp = subset_samples(resfinder_PHY_clean, name != "BSE100_S122" & name != "BSE79_S120" & name != "BSE74_S119" & name != "BSE93_S121")

# Check if there are taxa that are not present in any of the remaining samples
any(taxa_sums(resfinder_PHY_hosp) == 0)
resfinder_PHY_hosp <- prune_taxa(taxa_sums(resfinder_PHY_hosp) > 0, resfinder_PHY_hosp)
any(taxa_sums(resfinder_PHY_hosp) == 0)
```
## Ordinate ResFinder mapping results to study wheather the taxonomy can be explained by the variable "country"
```{r}
# Ordinate and plot data
resfinder_PHY_ord <- ordinate(resfinder_PHY_hosp, method = "PCoA", distance = "horn")
p3 <- plot_ordination(resfinder_PHY_hosp, resfinder_PHY_ord, color = "country", label = "name")
...
resfinder_temp <- subset_samples(resfinder_PHY_hosp, (country == "Finland" | country == "Benin"))
...
resfinder_temp <- subset_samples(resfinder_PHY_hosp, (country == "Burkina Faso" | country == "Benin"))
resfinder_dist <- vegdist(t(otu_table(resfinder_temp)), dist = "horn")
adonis(resfinder_dist ~ country, data = data.frame(sample_data(resfinder_temp), permutations = 9999))

resfinder_temp <- subset_samples(resfinder_PHY_hosp, (country == "Finland" | country == "Burkina Faso"))
resfinder_dist <- vegdist(t(otu_table(resfinder_temp)), dist = "horn")
adonis(resfinder_dist ~ country, data = data.frame(sample_data(resfinder_temp), permutations = 9999))
...
```{r}
Benin <- subset_samples(resfinder_PHY_hosp, (country == "Benin"))
Benin_dist <- vegdist(t(otu_table(Benin)), dist = "horn")
adonis(Benin_dist ~ location, data = data.frame(sample_data(Benin)), permutations = 99999)

BF <- subset_samples(resfinder_PHY_hosp, (country == "Burkina Faso"))
BF_dist <- vegdist(t(otu_table(BF)), dist = "horn")
adonis(BF_dist ~ location, data = data.frame(sample_data(BF)), permutations = 99999)

Finland <- subset_samples(resfinder_PHY_hosp, (country == "Finland"))
Finland_dist <- vegdist(t(otu_table(Finland)), dist = "horn")
adonis(Finland_dist ~ location, data = data.frame(sample_data(Finland)), permutations = 99999)
...
resfinder_PHY_ord <- ordinate(resfinder_PHY_hosp, method = "PCoA", distance = "horn")
p3.1 <- plot_ordination(resfinder_PHY_hosp, resfinder_PHY_ord, color = "location", label = "name")
...
# Test significance between all pairs
resfinder_temp <- subset_samples(resfinder_PHY_hosp, (location == "Savalou area" | location == "Central clinic of Calavi"))
resfinder_dist <- vegdist(t(otu_table(resfinder_temp)), dist = "horn")
...
# Shannon
## Take a look at the indexes
resfinder_tab <-microbiome::alpha(resfinder_PHY_hosp, index = "diversity_shannon")
kable(head(resfinder_tab))

## Create data table
resfinder.meta <- meta(resfinder_PHY_hosp)
...
# Gini Simpson
## Take a look at the indexes
resfinder_tab <-microbiome::alpha(resfinder_PHY_hosp, index = "diversity_gini_simpson")
kable(head(resfinder_tab))
...
# Inverse Simpson
## Take a look at the indexes
resfinder_tab <-microbiome::alpha(resfinder_PHY_hosp, index = "diversity_inverse_simpson")
kable(head(resfinder_tab))
...
loc <- plot_richness(resfinder_PHY_hosp, measures = c("Shannon", "Simpson"), 
...
lo_data <- data.frame(SUM = round(mean16S_counts * (sample_sums(otu_table(resfinder_PHY_hosp)))), 
    location = sample_data(resfinder_PHY_hosp)$location)
...
co_data <- data.frame(SUM = round(mean16S_counts * (sample_sums(otu_table(resfinder_PHY_hosp)))), 
    country = sample_data(resfinder_PHY_hosp)$country)
...
```
## ARG Classes, sum relative abundance by individual sample
```{r}
resfinder_PHY_Class <- tax_glom(resfinder_PHY_hosp, taxrank = "Class")
...
# When using length normalized data, the highest sum (BH58) is 0.4 while with classically normalized data it is 0.2.
```
...
```
## ARG Classes, sum relative abundance summed up by location / country
```{r}
...
```
## DRAFT ARG Classes, relative abundance by location
```{r}
resfinder_PHY_Class <- tax_glom(resfinder_PHY_clean, taxrank = "Class")
...
# 
#p22<-plot_bar(resfinder_PHY_Class_abund, x="location", fill="Class")
#resfinder.p22 <- p22 + facet_grid(~ location, scales = "free", space = "free", labeller = label_wrap_gen()) + 
#  ggtitle("Relative abundance of ARGs by classes/16S") +
#   theme_linedraw() +
#  theme(axis.text.x = element_text(angle=90, hjust=1, size = 8),
#        axis.title.x = element_blank(),
#        strip.text.x = element_text(size = 9, angle = 90, hjust = 0)) +
...
# Remove OTU separators
#resfinder.p22 + geom_bar(aes(color=Class, fill=Class), stat="identity", position="stack")
```
