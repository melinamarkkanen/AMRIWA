...
## rpoB
```{r, warning=FALSE, error=FALSE, message=FALSE, results='hide', fig.show='hide'}
HMM_RESULT_TABLE <- read.delim("HMM_RESULT_TABLE.txt", row.names=1)
...
rownames(counts_resfinder_phy) <- c(1:3104)
...
rownames(resfinder_length_SSU_norm) <- c(1:3104)
...
### Resistome
```{r}
resfinder_PHY_counts_HWW_trans = resfinder_PHY_counts_HWW
transformed_OTU_ARGs <- microbiome::transform(resfinder_PHY_counts_HWW_trans, "clr")
resfinder_PHY_counts_HWW_trans@otu_table <- otu_table(transformed_OTU_ARGs, taxa_are_rows = T)

resfinder_PHY_ord <- ordinate(resfinder_PHY_counts_HWW_trans, method = "PCoA", distance = "euclidean")
p_ord <- plot_ordination(resfinder_PHY_counts_HWW_trans, resfinder_PHY_ord, color = "country")
a <- p_ord + geom_point(size = 4, alpha = 0.5) +
  labs(title = "ARGs") + theme_bw() + 
  ...
  #geom_text_repel(mapping = aes(label = hospital), 
  #                size = 4, family = "Times", hjust = 1.2, vjust = 0.3) +
  stat_ellipse(level = 0.95) +
  scale_color_manual(values=c("#1C4731", "#910C28", "#1E88E5"))

# All samples
resfinder_PHY_counts_uniq_trans = resfinder_PHY_counts_uniq
transformed_OTU_ARGs <- microbiome::transform(resfinder_PHY_counts_uniq_trans, "clr")
resfinder_PHY_counts_uniq_trans@otu_table <- otu_table(transformed_OTU_ARGs, taxa_are_rows = T)

resfinder_PHY_ord <- ordinate(resfinder_PHY_counts_uniq_trans, method = "PCoA", distance = "euclidean")
p_ord <- plot_ordination(resfinder_PHY_counts_uniq_trans, resfinder_PHY_ord, color = "country")
a2 <- p_ord + geom_point(size = 4, alpha = 0.5,) +
  labs(title = "ARGs") + theme_bw() + 
  theme(text = element_text(family = "Palatino"),
    plot.title = element_text(size = 17, hjust = 0.5),
    axis.title = element_text(size = 15, face = "bold"),
...
#a2_plot + plot_annotation(tag_levels = list(c("A")), title = "PCoA, clr-transformed, Euclidean distance") & 
...
### Taxa
```{r}
metaphlan_PHY_counts_genus_HWW_trans = metaphlan_PHY_counts_genus_HWW
transformed_OTU_taxa <- microbiome::transform(metaphlan_PHY_counts_genus_HWW_trans, "clr")
metaphlan_PHY_counts_genus_HWW_trans@otu_table <- otu_table(transformed_OTU_taxa, taxa_are_rows = T)

metaphlan_PHY_ord <- ordinate(metaphlan_PHY_counts_genus_HWW_trans, method = "PCoA", distance = "euclidean")
p_ord <- plot_ordination(metaphlan_PHY_counts_genus_HWW_trans, metaphlan_PHY_ord, color = "country")
b <- p_ord + geom_point(size = 4, alpha = 0.5) +
  labs(title = "Taxa") + theme_bw() + 
  theme(text = element_text(family = "Palatino"),
    plot.title = element_text(size = 17, hjust = 0.5),
    axis.title = element_text(size = 15, face = "bold"),
    legend.title = element_blank(),
...
# All samples
metaphlan_PHY_counts_genus_uniq_trans = metaphlan_PHY_counts_genus_uniq
transformed_OTU_taxa <- microbiome::transform(metaphlan_PHY_counts_genus_uniq_trans, "clr")
metaphlan_PHY_counts_genus_uniq_trans@otu_table <- otu_table(transformed_OTU_taxa, taxa_are_rows = T)

metaphlan_PHY_ord <- ordinate(metaphlan_PHY_counts_genus_uniq_trans, method = "PCoA", distance = "euclidean")
p_ord <- plot_ordination(metaphlan_PHY_counts_genus_uniq_trans, metaphlan_PHY_ord, color = "country")
b2 <- p_ord + geom_point(size = 4, alpha = 0.5) +
  labs(title = "Taxa") + theme_bw() + 
  theme(text = element_text(family = "Palatino"),
    plot.title = element_text(size = 17, hjust = 0.5),
    axis.title = element_text(size = 15, face = "bold"),
...
#b2_plot + plot_annotation(tag_levels = list(c("B")), title = "PCoA, clr-transformed, Euclidean distance") & 
#  theme(plot.title = element_text(size = 25, family = "Palatino", face = "bold"),
#    plot.tag = element_text(size = 25))
...
### Mobilome
```{r}
MGE_PHY_counts_HWW_trans = MGE_PHY_counts_HWW
transformed_OTU_mge <- microbiome::transform(MGE_PHY_counts_HWW_trans, "clr")
MGE_PHY_counts_HWW_trans@otu_table <- otu_table(transformed_OTU_mge, taxa_are_rows = T)

MGE_PHY_ord <- ordinate(MGE_PHY_counts_HWW_trans, method = "PCoA", distance = "euclidean")
p_ord <- plot_ordination(MGE_PHY_counts_HWW_trans, MGE_PHY_ord, color = "country")
c <- p_ord + geom_point(size = 4, alpha = 0.5) +
  labs(title = "MGEs") + theme_bw() + 
  theme(text = element_text(family = "Palatino"),
    plot.title = element_text(size = 17, hjust = 0.5),
    axis.title = element_text(size = 15, face = "bold"),
    legend.title = element_blank(),
...
patchwork0 <- a + b + c + plot_layout(design = layout) + 
  ...
  title = "PCoA, clr-transformed, Euclidean distance") & 
...
# All samples
MGE_PHY_counts_uniq_trans = MGE_PHY_counts_uniq
transformed_OTU_mge <- microbiome::transform(MGE_PHY_counts_uniq_trans, "clr")
MGE_PHY_counts_uniq_trans@otu_table <- otu_table(transformed_OTU_mge, taxa_are_rows = T)

MGE_PHY_ord <- ordinate(MGE_PHY_counts_uniq_trans, method = "PCoA", distance = "euclidean")
p_ord <- plot_ordination(MGE_PHY_counts_uniq_trans, MGE_PHY_ord, color = "country")
c2 <- p_ord + geom_point(size = 4, alpha = 0.5) +
  labs(title = "MGEs") + theme_bw() + 
  theme(text = element_text(family = "Palatino"),
    plot.title = element_text(size = 17, hjust = 0.5),
    axis.title = element_text(size = 15, face = "bold"),
...
#c2_plot + plot_annotation(tag_levels = list(c("C")), title = "PCoA, clr-transformed, Euclidean distance") & 
#  theme(plot.title = element_text(size = 25, family = "Palatino", face = "bold"),
#    plot.tag = element_text(size = 25))
...
p <- ggplot(div, aes(x=country, y=observed, fill = country)) + 
  geom_boxplot()

#"#1C4731", "#910C28", "#1E88E5"
#"#9FBDAE", "#DF7087", "#A7CAE9"
#"#64B98F", "#CD3857", "#61A6E3"
p3 <- p + geom_dotplot(binaxis='y', stackdir='center', dotsize = 0.4) + 
...
clusters_tax_table_resfinder <- 
  read.csv("clusters_tax_table.txt", header=FALSE, sep=";")
colnames(clusters_tax_table_resfinder) <- c("Gene",  "Cluster_name", "Class")
# Reorder columns
col_order <- c("Class", "Cluster_name", "Gene")
clusters_tax_table_resfinder <- clusters_tax_table_resfinder[, col_order]
...
# Divide by ARG gene lengths
resfinder_lengths <- 
  read.delim("resfinder_lengths.txt", header=FALSE, comment.char="#")
all(rownames(clusters_tax_table_resfinder$Gene) == resfinder_lengths$V1)
...
# Exclude biological / technical replicates
resfinder_deseq_uniq <- subset_samples(resfinder_deseq, replicate == "NO")
resfinder_deseq_uniq <- subset_samples(resfinder_deseq_uniq, sample_material == "water")

# Create phyloseq object with only HWW samples
resfinder_deseq_HWW <- subset_samples(resfinder_deseq_uniq, HWW == "YES")

# Take pair wise comparisons
deseq_PHY = subset_samples(resfinder_deseq_HWW, country == "Benin" | country == "Finland")
...
Ben_Fin_deseq2 <- sigtab_resfinder[order(-sigtab_resfinder$log2FoldChange), ]

# Save
#write.table(Ben_Fin_deseq2, "Ben_Fin_deseq2.txt", 
...
x <- aldex.clr(ARG_counts, conds = conditions,mc.samples = 128, denom = "all", verbose = FALSE)
...
# Plot
#png("plot.png", width=8, height=8,units="in",res=500)
#par(mfrow = c(1, 2))
#aldex.plot(aldex_out, type = "MA", test = "wilcox", xlab = "Log-ratio abundance", ylab = "Difference", cutoff.pval = 0.05, cutoff.effect = 1)
#aldex.plot(aldex_out, type = "MW", test = "wilcox", xlab = "Dispersion", ylab = "Difference", cutoff.pval = 0.05, cutoff.effect = 1)
#dev.off()
...
# Shorten names
aldex2_CLR$gene <- gsub('_[0-9a-zA-Z-]{6,}', '', aldex2_CLR$gene)
aldex2_CLR = aldex2_CLR

Ben_Fin_aldex2_CLR_filt <- subset(aldex2_CLR, wi.eBH < 0.05 & (diff.btw >= 1 | diff.btw <= -1))
#BF_Fin_aldex2_CLR_filt <- subset(aldex2_CLR, wi.eBH < 0.05 & (diff.btw >= 1 | diff.btw <= -1))
#Ben_BF_aldex2_CLR_filt <- subset(aldex2_CLR, wi.eBH < 0.05 & (diff.btw >= 1 | diff.btw <= -1))

# Save
#write.table(Ben_Fin_aldex2_CLR_filt, "Ben_Fin_aldex2_CLR_filt.txt", 
#            row.names=F, sep = "\t", col.names = T)

# Save
#write.table(BF_Fin_aldex2_CLR_filt, "BF_Fin_aldex2_CLR_filt.txt", 
#            row.names=F, sep = "\t", col.names = T)

# Save
#write.table(Ben_BF_aldex2_CLR_filt, "Ben_BF_aldex2_CLR_filt.txt", 
#            row.names=F, sep = "\t", col.names = T)
```
### Aldex2, with ALR, rpoB, ARGs
```{r, warning=FALSE, error=FALSE, message=FALSE, results='hide'}
resfinder_PHY_counts_HWW_Ben_Fin <- subset_samples(resfinder_PHY_counts_HWW, country == "Benin" | country == "Finland")
...
# Reorder samples to match metadata
match <- match(resfinder_PHY_counts_HWW_Ben_Fin@sam_data@row.names, rownames(HMM_RESULT_TABLE))
rpoB_counts <- HMM_RESULT_TABLE[match,]

conditions <- factor(resfinder_PHY_counts_HWW_Ben_Fin@sam_data$country, levels=c("Benin", "Finland"))
all(rownames(rpoB_counts) ==  colnames(ARG_counts))

rpoB_counts_t <- t(rpoB_counts)

ARG_counts_df = ARG_counts
df <- rbind(ARG_counts_df, rpoB_counts_t[2,])
rownames(df)[nrow(df)] <- "R1"
...
aldex2_ALR_rpoB <- aldex_out[order(aldex_out$diff.btw),]
rownames(aldex2_ALR_rpoB) <- c(1:nrow(aldex2_ALR_rpoB))
aldex2_ALR_rpoB$analysis <- rep("Aldex2_ALR_rpoB", nrow(aldex2_ALR_rpoB))

# Shorten names
aldex2_ALR_rpoB$gene <- gsub('_[0-9a-zA-Z-]{6,}', '', aldex2_ALR_rpoB$gene)
aldex2_ALR_rpoB = aldex2_ALR_rpoB

Ben_Fin_aldex2_ALR_rpoB_filt <- subset(aldex2_ALR_rpoB, wi.eBH < 0.05 & (diff.btw >= 1 | diff.btw <= -1))
#BF_Fin_aldex2_ALR_rpoB_filt <- subset(aldex2_ALR_rpoB, wi.eBH < 0.05 & (diff.btw >= 1 | diff.btw <= -1))
#Ben_BF_aldex2_ALR_rpoB_filt <- subset(aldex2_ALR_rpoB, wi.eBH < 0.05 & (diff.btw >= 1 | diff.btw <= -1))

# Save
#write.table(Ben_Fin_aldex2_ALR_rpoB_filt, "Ben_Fin_aldex2_ALR_rpoB_filt.txt", 
#            row.names=F, sep = "\t", col.names = T)

# Save
#write.table(BF_Fin_aldex2_ALR_rpoB_filt, "BF_Fin_aldex2_ALR_rpoB_filt.txt", 
#            row.names=F, sep = "\t", col.names = T)

# Save
#write.table(Ben_BF_aldex2_ALR_rpoB_filt, "Ben_BF_aldex2_ALR_rpoB_filt.txt", 
#            row.names=F, sep = "\t", col.names = T)
```
### Aldex2, with ALR, SSU, ARGs
```{r, warning=FALSE, error=FALSE, message=FALSE, results='hide', fig.show='hide'}
## SSU_counts
resfinder_PHY_counts_HWW_Ben_Fin <- subset_samples(resfinder_PHY_counts_HWW, country == "Benin" | country == "Finland")
...
# Plot with volcano plot with only SSU
p1 <- ggplot(data=aldex_out, aes(x=diff.btw, y=-log10(wi.eBH), col=country, label=gene)) +
        ...
        #geom_vline(xintercept=c(-1, 1), col="#2842E6", linetype = "longdash") +
        geom_hline(yintercept=-log10(0.05), col="#2842E6", linetype = "longdash") + 
  theme(text = element_text(family = "Palatino"),
    axis.title.x = element_text(size = 20, face = "bold", hjust = 0.53),
    axis.text = element_text(size = 20),
    legend.position = "none",
    aspect.ratio = 1,
    plot.subtitle = element_text(size = 20, hjust = 0.45)) + coord_fixed() +
 #labs(subtitle = '          Benin             Finland', axis = "diff.btw") +
 labs(subtitle = '  Burkina Faso          Finland', axis = "diff.btw") +
 #labs(subtitle = '             Benin            Burkina Faso', axis = "diff.btw") + 
...
#            row.names=F, sep = "\t", col.names = T)
```
### Correlation studying differentially abundant ARGs
```{r, warning=FALSE, error=FALSE, message=FALSE, results='hide', fig.show='hide'}
# ALR SSU
Ben_Fin_deseq2 <- read.delim("Ben_Fin_deseq2.txt", stringsAsFactors=TRUE)
Ben_Fin_deseq2$gene <- gsub('_[0-9a-zA-Z-]{6,}', '', Ben_Fin_deseq2$gene)

Ben_Fin_aldex2_ALR_SSU_cp = Ben_Fin_aldex2_ALR_SSU_filt
match <- match(Ben_Fin_deseq2$gene, Ben_Fin_aldex2_ALR_SSU_cp$gene)
Ben_Fin_aldex2_ALR_SSU_cp <- Ben_Fin_aldex2_ALR_SSU_cp[match, ]

# Compare with deseq2
# Replace NA with 0
Ben_Fin_aldex2_ALR_SSU_cp["diff.btw"][is.na(Ben_Fin_aldex2_ALR_SSU_cp["diff.btw"])] <- 0
Ben_Fin_aldex2_ALR_SSU_cp$gene <- Ben_Fin_deseq2$gene
all(Ben_Fin_aldex2_ALR_SSU_cp$gene == Ben_Fin_deseq2$gene)
rownames(Ben_Fin_aldex2_ALR_SSU_cp) <- 1:nrow(Ben_Fin_aldex2_ALR_SSU_cp)

Ben_Fin_aldex2_ALR_SSU_cp <- Ben_Fin_aldex2_ALR_SSU_cp[, c(12, 8)]
 
df1 <- cbind(Ben_Fin_aldex2_ALR_SSU_cp, Ben_Fin_deseq2$log2FoldChange)
colnames(df1) <- c("gene", "Aldex2_SSU", "DESeq2")
dim(df1)
...
Ben_Fin_aldex2_ALR_rpoB_cp = Ben_Fin_aldex2_ALR_rpoB_filt
match <- match(Ben_Fin_deseq2$gene, Ben_Fin_aldex2_ALR_rpoB_cp$gene)
Ben_Fin_aldex2_ALR_rpoB_cp <- Ben_Fin_aldex2_ALR_rpoB_cp[match, ]

# Compare with deseq2
# Replace NA with 0
Ben_Fin_aldex2_ALR_rpoB_cp["diff.btw"][is.na(Ben_Fin_aldex2_ALR_rpoB_cp["diff.btw"])] <- 0
Ben_Fin_aldex2_ALR_rpoB_cp$gene <- Ben_Fin_deseq2$gene
all(Ben_Fin_aldex2_ALR_rpoB_cp$gene == Ben_Fin_deseq2$gene)
rownames(Ben_Fin_aldex2_ALR_rpoB_cp) <- 1:nrow(Ben_Fin_aldex2_ALR_rpoB_cp)

Ben_Fin_aldex2_ALR_rpoB_cp <- Ben_Fin_aldex2_ALR_rpoB_cp[, c(12, 8)]
 
df2 <- cbind(Ben_Fin_aldex2_ALR_rpoB_cp, Ben_Fin_deseq2$log2FoldChange)
colnames(df2) <- c("gene", "Aldex2_rpoB", "DESeq2")
dim(df2)
...
Ben_Fin_aldex2_CLR_cp = Ben_Fin_aldex2_CLR_filt
match <- match(Ben_Fin_deseq2$gene, Ben_Fin_aldex2_CLR_cp$gene)
Ben_Fin_aldex2_CLR_cp <- Ben_Fin_aldex2_CLR_cp[match, ]

# Compare with deseq2
# Replace NA with 0
Ben_Fin_aldex2_CLR_cp["diff.btw"][is.na(Ben_Fin_aldex2_CLR_cp["diff.btw"])] <- 0
Ben_Fin_aldex2_CLR_cp$gene <- Ben_Fin_deseq2$gene
all(Ben_Fin_aldex2_CLR_cp$gene == Ben_Fin_deseq2$gene)
rownames(Ben_Fin_aldex2_CLR_cp) <- 1:nrow(Ben_Fin_aldex2_CLR_cp)

Ben_Fin_aldex2_CLR_cp <- Ben_Fin_aldex2_CLR_cp[, c(12, 8)]
 
df3 <- cbind(Ben_Fin_aldex2_CLR_cp, Ben_Fin_deseq2$log2FoldChange)
colnames(df3) <- c("gene", "Aldex2_CLR", "DESeq2")

df <- cbind(df1, df2$Aldex2_rpoB, df3$Aldex2_CLR)
colnames(df) <- c( "gene", "ALDEx2_ALR_SSU", "DESeq2", "ALDEx2_ALR_rpoB", "ALDEx2_CLR")

#mdata <- melt(df)
#colnames(mdata) <- c("gene", "method", "value")

# Plot
c <- ggscatter(df, x = "ALDEx2_ALR_SSU", y = "DESeq2", 
          add = "reg.line", conf.int = TRUE,
          cor.coef = TRUE, cor.method = "pearson",
          add.params = list(color = "blue", fill = "lightgray"),
          xlab = "ALDEx2_ALR_SSU", ylab = "DESeq2", 
          cor.coef.size = 6.5, cor.coef.coord = c(-7, 8), font.family	= "Palatino") + 
  theme(text = element_text(family = "Palatino"),
    axis.title = element_text(size = 20),
    axis.text = element_text(size = 15),
    aspect.ratio = 1) +
geom_point(data=df[which(df$ALDEx2_CLR == 0),], color = "darkgrey", shape = 21, size = 4, fill = "white") + 
geom_point(data=df[which(df$ALDEx2_CLR != 0),], color = "black", shape = 21, size = 4, fill = "white")
c

design = "
ABC
"
patch <- a + b + c + plot_annotation(
  title = "Correlations between results using different approaches to study differentially abundant ARGs", tag_levels = "A") & 
  theme(text = element_text(family = "Palatino"),
        plot.title = element_text(size = 25, face = "bold"),
        plot.tag = element_text(size = 25, face = "bold"))
patch + plot_layout(design = design)
...
# (= the ones detected by DESeq2 but not other methods.)
df_zeros <- subset(df, ALDEx2_ALR_SSU == 0 | ALDEx2_ALR_rpoB == 0 | ALDEx2_CLR == 0)

# (= the ones that are found by all methods)
df_all <- subset(df, ALDEx2_ALR_SSU != 0 & ALDEx2_ALR_rpoB != 0 & ALDEx2_CLR != 0 & DESeq2 != 0)
```
...
#M <- glm(RA~country, data = df, family = "Gamma"(link="log") )
summary(M)
#summary(M)$coef
shapiro.test(df$RA)
...
# Kruskal-Wallis test can be performed for means not for medians.
...
```{r, warning=FALSE, error=FALSE, message=FALSE, results='hide'}
df<-data.frame(SUM=round(mean(sample_data(MGE_rel_PHY_HWW)$SSU)*(sample_sums(MGE_rel_PHY_HWW))),
...
# Kruskal-Wallis test could be performed for medians.
...
pl <- p + geom_boxplot(width=0.3, fill = c("#9FBDAE", "#DF7087", "#A7CAE9"), outlier.shape = NA, alpha = 0.5)
plo <- pl +  geom_jitter(data = subset(df, rownames(df) != "BFH19_S137" & rownames(df) != "BFH25_S143" & rownames(df) != "BFH35_S153"), 
                         aes(x = country, y = RA, color = country), size = 7, alpha = 0.7, width = 0.15)
...
car::ncvTest(M)  # Breusch-Pagan test
# There is no heteroscedasticity. 
# Kruskal-Wallis test could be performed for means, but not medians.
...
patch <- title + ARG_plot + MGE_plot + intI1_plot + 
...
patch <-ARG_plot + MGE_plot + intI1_plot +
  plot_layout(nrow = 1) + 
...
#       width = 16, height = 13, dpi = 300, units = "in", device='png', scale = 1)
```
### Carbapenemases, five plots
```{r, warning=FALSE, error=FALSE, message=FALSE, results='hide', fig.show='hide'}
# Five separate plots into one
...
mcr <- data.frame("mcr-1" = sample_sums(resfinder_PHY_mcr_1),
                  "mcr-2.1" = sample_sums(resfinder_PHY_mcr_2),
                  "mcr-3.1" = sample_sums(resfinder_PHY_mcr_3.1),
                  "mcr-3.17" = sample_sums(resfinder_PHY_mcr_3.17),
                  "mcr-4.1" = sample_sums(resfinder_PHY_mcr_4),
                  "mcr-5" = sample_sums(resfinder_PHY_mcr_5),
                  "mcr-6" = sample_sums(resfinder_PHY_mcr_6),
                  "mcr-7" = sample_sums(resfinder_PHY_mcr_7),
                  "mcr-8" = sample_sums(resfinder_PHY_mcr_8),
                  "mcr-9" = sample_sums(resfinder_PHY_mcr_9),
                  "mcr-10" = sample_sums(resfinder_PHY_mcr_10))
```
...
aldex_out$country <- ifelse(aldex_out$diff.btw < 0,  'Burkina Faso',  'Finland')
...
aldex_out$country <- ifelse(aldex_out$diff.btw < 0,  'Burkina Faso',  'Benin')
...
#       width = 16, height = 13, dpi = 300, units = "in", device='png', scale = 1)
```
## Correlation between ARG & intI
```{r, warning=FALSE, error=FALSE, message=FALSE, results='hide', fig.show='hide'}
MGE_PHY_counts_HWW_intI1 <- subset_taxa(MGE_PHY_counts_HWW, Class == "intI1")
MGE_PHY_counts_HWW_intI1 <- subset_taxa(MGE_PHY_counts_HWW, Class == "qacEdelta")
MGE_PHY_counts_HWW_intI1 <- subset_taxa(MGE_PHY_counts_HWW, Class == "int2")
MGE_PHY_counts_HWW_intI1 <- subset_taxa(MGE_PHY_counts_HWW, Class == "int3")
MGE_PHY_counts_HWW_intI1 = MGE_PHY_counts_HWW

df1 <- data.frame("ARGs" = sample_sums(resfinder_PHY_counts_HWW))
df2 <- data.frame("MGEs" = sample_sums(MGE_PHY_counts_HWW_intI1))
df <- cbind(df1, df2)
df$country <- MGE_PHY_counts_HWW_intI1@sam_data$country

#"#9FBDAE", "#DF7087", "#A7CAE9"
#"#1C4731", "#910C28", "#1E88E5"
#"#BFD1C8", "#EEBDC7", "#CEE1F3"

p5 <- ggscatter(df, x = "ARGs", y = "MGEs", add = "reg.line", conf.int = TRUE,
          color = "country", palette = c("#1C4731", "#910C28", "#1E88E5"),
          alpha = 0.6, size = 3, show.legend = FALSE) +
  stat_cor(aes(color = country), label.x = 1.5, size = 5.5, family = "Palatino", show.legend = FALSE) + 
  theme(text = element_text(family = "Palatino"),
...
#  plot_annotation(title = "Correlation between ARGs and MGEs in HWWs", tag_levels = list(c('A', 'B', 'C', 'D'))) +
#plot_layout(ncol = 2) &
#theme(text = element_text(family = "Palatino"),
#        legend.position = "none",
