...
### PCOA, resistome
```{r, warning=FALSE, error=FALSE, message=FALSE, results='hide'}
# resfinder_PHY_rel_uniq
# resfinder_PHY_rel_HWW

resfinder_PHY_ord <- ordinate(resfinder_PHY_rel_HWW, method = "PCoA", distance = "bray")
p_ord <- plot_ordination(resfinder_PHY_rel_HWW, resfinder_PHY_ord, color = "country")
...
tmp <- subset_samples(resfinder_PHY_rel_HWW, country == "Burkina Faso" | country == "Benin")
dist <- vegdist(t(otu_table(tmp)), dist = "bray")
vegan::adonis2(dist ~country, data = data.frame(sample_data(tmp), permutations = 9999))

p <- c("0.001", "0.001", "0.001")
p.adjust(p, method = "BH", n = length(p))

# All samples
resfinder_PHY_ord <- ordinate(resfinder_PHY_rel_uniq, method = "PCoA", distance = "bray")
p_ord <- plot_ordination(resfinder_PHY_rel_uniq, resfinder_PHY_ord, color = "country")
...
a2_plot + plot_annotation(tag_levels = list(c("A")), title = "PcoA, Bray-Curtis distance") & 
...
### PCOA, taxa
```{r, warning=FALSE, error=FALSE, message=FALSE, results='hide'}
#metaphlan_PHY_rel_genus_HWW
#metaphlan_PHY_counts_genus_HWW
metaphlan_PHY_ord <- ordinate(metaphlan_PHY_rel_genus_HWW, method = "PCoA", distance = "bray")
p_ord <- plot_ordination(metaphlan_PHY_rel_genus_HWW, metaphlan_PHY_ord, color = "country")
...
tmp <- subset_samples(metaphlan_PHY_rel_genus_HWW, country == "Benin" | country == "Burkina Faso")
dist <- vegdist(t(otu_table(tmp)), dist = "bray")
vegan::adonis2(dist ~country, data = data.frame(sample_data(tmp), permutations = 9999))

p <- c("0.001", "0.041", "0.001")
p.adjust(p, method = "BH", n = length(p))

# All samples
metaphlan_PHY_ord <- ordinate(metaphlan_PHY_rel_genus_uniq, method = "PCoA", distance = "bray")
p_ord <- plot_ordination(metaphlan_PHY_rel_genus_uniq, metaphlan_PHY_ord, color = "country")
...
b2_plot + plot_annotation(tag_levels = list(c("B")), title = "PcoA, Bray-Curtis distance") & 
  theme(plot.title = element_text(size = 25, family = "Palatino", face = "bold"),
    plot.tag = element_text(size = 25))
...
```{r, warning=FALSE, error=FALSE, message=FALSE, results='hide'}
# PCoA
MGE_PHY_ord <- ordinate(MGE_rel_PHY_HWW, method = "PCoA", distance = "bray")
p_ord <- plot_ordination(MGE_rel_PHY_HWW, MGE_PHY_ord, color = "country")
...
tmp <- subset_samples(MGE_rel_PHY_HWW, country == "Burkina Faso" | country == "Benin")
dist <- vegdist(t(otu_table(tmp)), dist = "bray")
vegan::adonis2(dist ~country, data = data.frame(sample_data(tmp), permutations = 9999))
...
patchwork0 <- a + b + c + plot_layout(design = layout) + 
  plot_annotation(tag_levels = list(c('A', 'B', 'C')), title = "PcoA, Bray-Curtis distance") & 
...
# All samples
MGE_PHY_ord <- ordinate(MGE_PHY_counts_uniq, method = "PCoA", distance = "bray")
p_ord <- plot_ordination(MGE_PHY_counts_uniq, MGE_PHY_ord, color = "country")
...
c2 <- c2_plot + plot_annotation(tag_levels = list(c("C")), title = "PcoA, Bray-Curtis distance") & 
  theme(plot.title = element_text(size = 25, family = "Palatino", face = "bold"),
    plot.tag = element_text(size = 25))
...
```{r, warning=FALSE, error=FALSE, message=FALSE, results='hide'}
## SSU_counts
resfinder_PHY_counts_HWW_Ben_Fin <- subset_samples(resfinder_PHY_counts_HWW, country == "Benin" | country == "Burkina Faso")
...
conditions <- factor(resfinder_PHY_counts_HWW_Ben_Fin@sam_data$country,levels=c("Benin", "Burkina Faso"))
...
## OR ##
#conds<- factor(sample_data(resfinder_PHY_counts_HWW_Ben_Fin)$country,levels=c("Benin", "Burkina Faso"))
#tt<- aldex(reads=ceiling(df),conditions=conds,denom=ref)
## OR ##
...
p3 <- ggplot(data=aldex_out, aes(x=diff.btw, y=-log10(wi.eBH), col=country, label=gene)) +
...
# labs(subtitle = bquote('            Benin     ' ~ bold('diff.btw')~'      Finland')) +
# labs(subtitle = bquote('Burkina Faso ' ~bold('diff.btw')~' Finland')) +
 labs(subtitle = bquote('                   Benin  ' ~ bold('diff.btw')~' Burkina Faso')) + 
...
patch <- 
  ...
  plot_annotation(title = "Divergent ARGs (ALDEx2) in HWWs from different countries\n",
                  tag_levels = "A") & 
...
#            row.names=F, sep = "\t", col.names = T)
```
### Correlation studying divergent ARGs
```{r, warning=FALSE, error=FALSE, message=FALSE, results='hide'}
# ALR SSU
...
patch <- a + b + c + plot_annotation(
  title = "Correlations between results using different approaches to study divergent ARGs", tag_levels = "A") & 
  theme(text = element_text(family = "Palatino"),
...
# Plot
HighlightData <- d1[(d1$Genus == "Acinetobacter" | d1$Genus == "Comamonas" | d1$Genus == "Thauera" | d1$Genus == "Proteobacteria_unclassified" |
                     d1$Genus ==  "Citrobacter" | d1$Genus == "Paracoccus" | d1$Genus == "Enhydrobacter" |  d1$Genus == "Moraxella" |
                      d1$Genus == "Raoultella" | d1$Genus == "Delftia"),]
...
patch <- a + b + c + plot_layout(design = design) + 
  plot_annotation(title = "Divergent taxa in HWWs (ALDEx2, clr-transformation)", tag_levels = 'A') & 
